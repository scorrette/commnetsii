...
from staticTables import nodes, links
    ...
    for x in range(v_count):
        for y in range(v_count):
            for link in links:  # iterates over each tuple which represents a link
                if (nodes_list[x] == links[0] and nodes_list[y] == link[1]) or (
                        nodes_list[y] == link[0] and nodes_list[x] == link[1]):
                    g.graph[x][y] = 1
                    g.graph[y][x] = 1
                elif x == y:
                    g.graph[x][y] = 0
...
def getNodeHopMap():
    ...
    for nodeName in nodes[TOPO_NUM]:
